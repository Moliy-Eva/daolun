...
#define SSID        "Moliy" //改为你的热点名称, 不要有中文
#define PASSWORD    "11012001"//改为你的WiFi密码Wi-Fi密码
#define DEVICEID    "562109393" //OneNet上的设备ID
String apiKey = "da7mS7OtEo1xZZPMbFkg08JMkIo=";//与你的设备绑定的APIKey
...
void loop(void)     //循环函数  
{
  ...
  if (state == 1)
  {
    Serial.println("Children is in this area!");
    if (sensor_tem>=MAX)
    { 
     Serial.println("高温警告，救救孩子！");
     tone(buzzer_pin,buzzer_fre); 
     delay(1000);//驱动蜂鸣器
     }
   } 
  else
    Serial.println("No one!");//***红外****
...
jsonToSend="{\"Temperature\":";      ////向Onenet传数据
